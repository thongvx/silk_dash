...
//retry transfer
const formRetryTransfer = `<div class="delete" id="retry-transfer">
                                    <form action="">
...
$(document).on('click', '[btn-retry-transfer]', function () {
    ...
    $('#retry-transfer form').on('submit', function(e) {
        ...
        $.ajax({
            ...
            success: function (response) {
                fixedBox()
                tr.find('.sv-transfer').text('0');
                tr.find('.url').removeClass('text-orange-500 text-rose-600 text-teal-500');
