                                    ...
                                    <div class="px-2 pt-4 md:p-4">
                                        ...
                                        <div class="flex justify-between items-center w-full mb-3">
                                            <form action="{{ route("video.index")}}" method="GET" class="text-sm bg-slate-900 rounded-lg p-2">
                                                ...
                                                <span>entries</span>
                                                <input name="folderId" type="text" value="{{ $currentFolderName -> id }}" class="hidden">
                                            </form>
                                        ...
                                        <div id="live"
                                            ...
                                            <div class="px-0 pt-0 overflow-auto max-h-[calc(100vh-21em)] ">
                                     ...
                                     before:inline-block before:font-normal before:text-[#1a2035] before:antialiased before:transition-all before:text-xl before:content-['▲'] sm:-mr-6                         lg:absolute lg:right-6 lg:left-auto lg:mt-2 lg:block lg:cursor-pointer">
                                            ...
                                            <div class="button-table pt-6 text-white text-sm flex justify-between items-center">
                                                ...
                                                <div class="pagination flex items-center">
                                                    ...
                                                    @else
                                                        <a class="hover:bg-emerald-400 py-2 px-4 w-max rounded-lg bg-slate-900" href="{{ $videos->previousPageUrl() }}&folderId={{$currentFolderName -> id}}&limit={{ $videos->perPage() }}" rel="prev">Previous</a>
                                                    @endif
                                                    ...
                                                    @if ($videos->currentPage() > 2)
                                                        <li class="list-none ">
                                                            <a class="hover:bg-emerald-400 text-white mx-1 py-2 px-3 w-max rounded-lg bg-slate-900" href="{{ $videos->url(1) }}&folderId={{$currentFolderName -> id}}&limit={{ $videos->perPage() }}">1</a>
                                                        </li>
                                                    ...
                                                    @for ($i = max(1, $videos->currentPage() - 1); $i <= min($videos->lastPage(), $videos->currentPage() + 1); $i++)
                                                        <li class="list-none page-item">
                                                            <a class="text-white mx-1 py-2 px-3 w-max rounded-lg {{ ($videos->currentPage() == $i) ? 'bg-emerald-400 cursor-not-allowed ' : 'bg-slate-900 hover:bg-emerald-400' }}" href="{{ $videos->url($i) }}&folderId={{$currentFolderName -> id}}&limit={{ $videos->perPage() }}">{{ $i }}</a>
                                                        </li>
                                                    @endfor
                                                    ...
                                                    @if ($videos->currentPage() < $videos->lastPage() - 1)
                                                        ...
                                                        @endif
                                                        <li class="list-none page-item">
                                                            <a class="hover:bg-emerald-400 text-white mx-1 py-2 px-3 w-max rounded-lg bg-slate-900" href="{{ $videos->url($videos->lastPage()) }}&folderId={{$currentFolderName -> id}}&limit={{ $videos->perPage() }}">{{ $videos->lastPage() }}</a>
                                                        </li>
                                                    @endif

                                                    {{-- Next Page Link --}}
                                                    @if ($videos->hasMorePages())
                                                        <a class="hover:bg-emerald-400 py-2 px-4 w-max rounded-lg bg-slate-900" href="{{ $videos->nextPageUrl() }}&folderId={{$currentFolderName -> id}}&limit={{ $videos->perPage() }}" rel="next">Next</a>
                                                    @else
                ...
                <div class="w-full overflow-hidden">
                    @foreach($folders as $folder)
                        @if ($folder -> name_folder === $currentFolderName -> name_folder)
                            <div class="item-folder rounded-lg text-white flex justify-between px-2 py-3 mb-2 bg-emerald-500">
