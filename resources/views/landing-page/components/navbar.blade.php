...
<nav class="{{ request()->path() == 'api' ? 'sticky top-0' : 'flex' }} z-20 items-center py-2 xl:px-20 px-4 sm:px-8 w-full text-[#009FB2] bg-[#142132]"
    ...
    <div class="flex items-center justify-between w-full px-0 py-1 flex-wrap">
        <div class="flex justify-center w-full sm:block sm:w-auto scale-75 md:scale-100">
            <a class="flex m-0 text-sm whitespace-nowrap items-center" href="/" target="_blank" logo>
                <img src="{{ asset('image/logo/logo4.webp') }}"
                     class="h-full max-w-full transition-all duration-200 ease-nav-brand max-h-12"
                     alt="main_logo"/>
        ...
        <div class="absolute top-16 bg-[#142132] w-full py-4 hidden md:block md:relative sm:py-0 sm:top-0 sm:w-auto"  dropdown-menu>
