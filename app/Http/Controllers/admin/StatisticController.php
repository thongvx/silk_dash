...
class StatisticController extends Controller
{
    ...
    private function getDataDate($tab, $date,$today, $earningToday, $startDate, $endDate)
    {
        ...
        if(Carbon::parse($endDate)->format('Y-m-d') == $today){
            ...
            $total = $totalViewsKey ? Redis::mget($totalViewsKey) : [];
            $totalViews = array_sum(array_filter($total, 'is_numeric')); // Bỏ qua các giá trị không hợp lệ
            ...
            $totalImpressionView1 = $totalImpression1 ? Redis::mget($totalImpression1) : [];
            $totalImpressionView2 = $totalImpression2 ? Redis::mget($totalImpression2) : [];

            $totalImpressionViews = array_sum(array_filter($totalImpressionView1, 'is_numeric')) +
                array_sum(array_filter($totalImpressionView2, 'is_numeric'));
